...
Anthropic API key by defaulting to a mock LLM when no key is provided.
...
class HealthAIAssistant:
    ...
    def __init__(self, anthropic_api_key: str = "mock-api-key"):
        # Initialize LLM only if a real key is provided and libraries are available
        if LANGGRAPH_AVAILABLE and anthropic_api_key != "mock-api-key":
            # Use a lower temperature and top_k sampling for more deterministic,
            # friendly outputs in a healthcare setting. top_k restricts token
            # sampling to the K most likely candidates, improving control over
            # wording【126651186224102†L494-L498】. A temperature around 0.5
            # balances creativity and precision【901531977100427†L81-L88】.
