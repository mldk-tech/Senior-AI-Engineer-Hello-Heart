...
- Sound robotic or impersonal"""
    ...
    def _generate_general_response(self, data: Dict) -> str:
        """
        Generate a general response when the user’s query doesn’t match a
        specific intent category.  Include personalised context from the
        user’s data (if available) and retrieved knowledge to encourage
        engagement and suggest next steps.

        Args:
            data: A dictionary of user health information.  Keys may include
                blood_pressure, steps_today, sleep_quality, hrv, knowledge, etc.

        Returns:
            A short, conversational response with a personalised touch.
        """
        parts = []
        # Build a short summary from available health metrics
        if 'steps_today' in data:
            steps = data['steps_today']
            parts.append(f"you’ve taken {steps:,} steps today")
        if 'sleep_quality' in data:
            parts.append(f"your sleep quality last night was {data['sleep_quality']}%")
        if 'hrv' in data:
            parts.append(f"your HRV is {data['hrv']} ms")
        if 'blood_pressure' in data:
            bp = data['blood_pressure']
            if isinstance(bp, dict) and 'systolic' in bp and 'diastolic' in bp:
                parts.append(f"your latest blood pressure is {bp['systolic']}/{bp['diastolic']} mmHg")

        summary = ''
        if parts:
            summary = "Based on your data, " + ", and ".join(parts) + ". "

        # Include retrieved knowledge context if present
        knowledge_text = ''
        if 'knowledge' in data:
            info = data['knowledge']
            words = info.split()
            truncated = ' '.join(words[:50])  # keep the knowledge snippet concise
            knowledge_text = f"Here’s some background: {truncated}... "

        return (
            f"I'm here to help you understand your health better. {summary}{knowledge_text}"
            "Which area would you like to focus on – your activity, sleep, or blood pressure?"
        )
...
class KnowledgeRetrievalAgent(BaseHealthAgent):
    """Retrieves general health information from a simple knowledge base.

    This agent is a simplified Retrieval‑Augmented Generation (RAG) component.  In a
    production system, a vector database and embedding model would be used to retrieve relevant
    documents.  Here, we use a keyword‑based lookup on a small knowledge base for demonstration.
    """

    # A simple hard‑coded knowledge base.  Keys are lowercase keywords and values are summaries.
    KNOWLEDGE_BASE = {
        "hypertension": (
            "Hypertension (high blood pressure) is a condition in which the force of the blood "
            "against your artery walls is too high. It's often called the 'silent killer' because "
            "it may have no symptoms but can lead to serious health problems if untreated."
        ),
        "blood pressure": (
            "Blood pressure is the pressure of circulating blood on the walls of blood vessels. "
            "It is expressed as two numbers: systolic (pressure when the heart beats) and diastolic "
            "(pressure when the heart rests). A normal reading is typically around 120/80 mmHg."
        ),
        "sleep": (
            "Adequate sleep is essential for good health. Most adults need 7–8 hours of quality sleep per "
            "night. Poor sleep can increase stress, blood pressure and risk of cardiovascular issues."
        ),
        "steps": (
            "Regular physical activity, like walking, helps maintain a healthy weight, lower blood pressure "
            "and improve mood. Aim for at least 150 minutes of moderate activity per week or around 10,000 "
            "steps per day."
        ),
    }
    ...
    def get_system_prompt(self) -> str:
        return (
            "You are a Knowledge Retrieval Agent in a deep‑agent health assistant. "
            "Use the shared context to look up relevant topics and write a short "
            "summary back into the context. Never make up information; only use the "
            "knowledge base you have been given."
        )

    def process(self, state: HealthAssistantState) -> HealthAssistantState:
        """Search the knowledge base for relevant content based on the last user message."""
        query = state["messages"][-1]["content"].lower()
        retrieved = None
        for keyword, summary in self.KNOWLEDGE_BASE.items():
            if keyword in query:
                retrieved = summary
                break
        if retrieved:
            # store the retrieved summary in the state for downstream agents
            state.setdefault("knowledge_context", retrieved)
        return state
...
class HealthAIAssistant:
    ...
    def __init__(self, openai_api_key: str = "enter your api key here"):
        ...
        # New knowledge retrieval agent for agentic RAG
        self.knowledge_agent = KnowledgeRetrievalAgent(self.llm)
    ...
    def process_message(self, user_message: str, thread_id: str = "default") -> str:
        ...
        if LANGGRAPH_AVAILABLE and self.workflow:
            ...
            if state["current_intent"] == "EMERGENCY":
                ...
                # Trigger knowledge retrieval for general health queries or medical advice
                if state["current_intent"] in ("HEALTH_QUERY", "MEDICAL_ADVICE"):
                    state = self.knowledge_agent.process(state)
                    # If knowledge was found, add it to user_health_data for the response agent
                    if "knowledge_context" in state:
                        state.setdefault("user_health_data", {})["knowledge"] = state["knowledge_context"]
