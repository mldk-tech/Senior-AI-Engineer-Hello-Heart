# Working Code Prototype - Health AI Assistant with Multi-Agent Architecture
# Demonstrates all 7 specialized agents using prompt engineering techniques
...
# Base Agent Class with Prompt Engineering
...
Respond with ONLY the category name, nothing else."""
    ...
    def process(self, state: HealthAssistantState) -> HealthAssistantState:
        ...
        # Use LLM for classification (mock for demo)
        context = {"user_message": last_message}
...
class DataRetrievalAgent(BaseHealthAgent):
    ...
    def __init__(self, user_data: Dict, llm: Optional[ChatAnthropic] = None):
        super().__init__(llm)
...
- Any notable changes"""
    ...
    def process(self, state: HealthAssistantState) -> HealthAssistantState:
        ...
        if intent == "BP_MONITORING":
            ...
            # For general queries, include summary of all data
            relevant_data = {
    ...
    def _generate_bp_response(self, data: Dict) -> str:
        if "blood_pressure" in data:
            ...
            return (f"Your latest reading is {bp['systolic']}/{bp['diastolic']} mmHg - "
                   f"that's in a healthy range! Your trend shows {data['blood_pressure']['trend']} "
                   f"progress. Keep up your healthy habits. When did you last take your medication?")
...
- EMERGENCY_VERIFIED: Proper emergency handling"""
    ...
    def process(self, state: HealthAssistantState) -> HealthAssistantState:
        ...
        if state["requires_medical_disclaimer"] and "not medical advice" not in last_response:
...
# Agent 5: Follow-up Determination Agent
...
Response format: {type: "TYPE", message: "Personalized message", schedule: "timing"}"""
    ...
    def process(self, state: HealthAssistantState) -> HealthAssistantState:
        ...
        # Analyze patterns for follow-up needs
        if "steps" in health_data:
            progress = float(health_data.get("weekly_progress", "0%").rstrip("%"))
            if progress < 50:
                state["follow_up_needed"] = True
                state["proactive_nudge"] = ("You're at {:.0f}% of your weekly step goal. "
                                           "A 10-minute walk could boost your energy! "
                                           "Shall I remind you later?").format(progress)
        
        if "sleep" in health_data:
            quality = health_data["sleep"]["last_night"]["quality_score"]
            if quality < 60:
                state["follow_up_needed"] = True
                state["proactive_nudge"] = ("Your sleep quality was lower last night. "
                                           "Would you like some tips for better rest tonight?")
                
        return state

# Agent 6: Proactive Engagement Agent
...
- Respect user preferences"""
    ...
    def process(self, state: HealthAssistantState) -> HealthAssistantState:
        if state.get("follow_up_needed") and state.get("proactive_nudge"):
            # Schedule the nudge (in production, this would use a task queue)
            print(f"\n[PROACTIVE NUDGE SCHEDULED]: {state['proactive_nudge']}")
            
        return state

# Agent 7: Emergency Response Agent
...
NEVER delay emergency response for any reason."""
    ...
    def process(self, state: HealthAssistantState) -> HealthAssistantState:
        if state["current_intent"] == "EMERGENCY":
            # Override any other response with emergency protocol
            emergency_response = ("🚨 I'm very concerned about your symptoms. "
...
class HealthAIAssistant:
    ...
    def __init__(self, anthropic_api_key: str):
        # Initialize LLM (mock for demo)
        self.llm = ChatAnthropic(
            api_key=anthropic_api_key,
            model="claude-3-sonnet-20240229",
            temperature=0.7,
            max_tokens=300
        ) if anthropic_api_key != "mock-api-key" else None
...
def demo_health_assistant():
    """Demonstrate all 7 agents working together"""
    assistant = HealthAIAssistant(anthropic_api_key=os.getenv("ANTHROPIC_API_KEY", "mock-api-key"))
    
    print("=== Hello Heart AI Assistant - Multi-Agent Demo ===\n")
    print("Demonstrating all 7 specialized agents:\n")
    print("1. Intent Classification Agent")
    print("2. Data Retrieval Agent")
    print("3. Response Generation Agent")
    print("4. Safety Check Agent")
    print("5. Follow-up Determination Agent")
    print("6. Proactive Engagement Agent")
    print("7. Emergency Response Agent")
    print("\n" + "="*50 + "\n")
